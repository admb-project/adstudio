...
;; 6.13 LaTeX
;;------------
(defun adstudio-LaTeX-hook ()
  (setq font-latex-fontify-script nil)
  (set-face-attribute 'font-latex-sectioning-0-face nil :height - )
  (set-face-attribute 'font-latex-sectioning-1-face nil :height - )
  (set-face-attribute 'font-latex-sectioning-2-face nil :height - )
  (set-face-attribute 'font-latex-sectioning-3-face nil :height - )
  (set-face-attribute 'font-latex-sectioning-4-face nil :height - )
  (set-face-attribute 'font-latex-sectioning-5-face nil :inherit -)
  (local-set-key [f7]  'AUCTeX-index  )
  (local-set-key [f8]  'AUCTeX-bibtex )
  (local-set-key [f9]  'AUCTeX-compile)
  (local-set-key [f10] 'AUCTeX-view   )
  (defun AUCTeX-compile ()
    "Compile LaTeX document." (interactive)
    (TeX-save-document (TeX-master-file))(TeX-command "LaTeX" 'TeX-master-file))
  (defun AUCTeX-bibtex ()
    "Compile LaTeX bibliography." (interactive)
    (TeX-command (if LaTeX-using-Biber "Biber" "BibTeX") 'TeX-master-file))
  (defalias 'AUCTeX-view 'TeX-view)
  (defun AUCTeX-index ()
    "Compile LaTeX index." (interactive)(TeX-command "Index" 'TeX-master-file)))
(add-hook 'LaTeX-mode-hook 'adstudio-LaTeX-hook)
;;--------
;; 6.16 R
;;--------
